...
| `-` | `--redact-sensitive [PREFIX SUFFIX]` | off | Redact sensitive values in normal logs. Without values, IDs are shown as `***`, while names, emoji labels, and paths are fully hidden as `***`. |
...
Redacts sensitive values in normal CLI logs and JSON logs.

Examples:

- `--redact-sensitive` fully masks IDs as `***`
- `--redact-sensitive 0 4` keeps only the last 4 characters of IDs, such as `***5678`
- `--redact-sensitive 4 4` keeps the first and last 4 characters of IDs, such as `1234***5678`
...
This currently affects operational logs such as:

- authenticated user lines
- usernames
- channel and DM names
- guild, channel, message, and user IDs
- emoji names and reaction identifiers in reaction-related logs
- preserve-cache paths

Examples of the resulting output:

- username or channel name: `***`
- channel ID with `--redact-sensitive 0 4`: `***5678`
- message ID with `--redact-sensitive 4 4`: `1234***5678`

Discovery output from [`--list-guilds`](#discovery) and [`--list-channels`](#discovery) is intentionally left unchanged, because those commands exist to reveal IDs and names. The flag is for operational logging, not discovery output.
