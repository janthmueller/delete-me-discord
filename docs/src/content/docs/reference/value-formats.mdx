...
For `include_ids` and `exclude_ids`, every value must be a complete decimal Discord ID.
Profile commands validate them through Discord before writing config. Partial ID suffixes are not accepted in v3.

`exclude_channel_types` accepts canonical values from `dmd list channel-types`. `exclude_thread_states` accepts values from `dmd list thread-states`.

Hand-written profile config accepts the same list shapes:

```json
{
  "include_ids": ["123456789012345678", "234567890123456789"]
}
```
