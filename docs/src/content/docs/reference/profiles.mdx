...
For `include_ids` and `exclude_ids`, profile commands accept complete decimal Discord IDs.
They validate those values through Discord before storing them in `config.json`. Partial ID suffixes are not accepted in v3.
...
--set keep_within=2w
--set preserve_cache=true
--set include_ids=123456789012345678,234567890123456789
--set exclude_channel_types=GuildVoice,PrivateThread
--set exclude_thread_states=archived
--set exclude_threads=true
--set delete_owned_threads=self-only
--set skip_unrestorable_threads=true
--set retry_time_buffer=0.1,0.3
