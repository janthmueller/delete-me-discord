...
- whether you enabled [`--redact-sensitive`](../cli-options/#--redact-sensitive)
...
Use [`--redact-sensitive`](../cli-options/#--redact-sensitive) when you want to share logs without exposing private names, IDs, or local paths.

When redaction is enabled:

- IDs are shown with the configured prefix/suffix window.
...
- `--redact-sensitive 4 4`:
  - channel ID -> `1234***5678`
  - channel name -> `***`

This applies to normal CLI logs and JSON logs. It does not change discovery output from [`--list-guilds`](../cli-options/#discovery) or [`--list-channels`](../cli-options/#discovery), because those commands are specifically meant to show names and IDs.
