...
Replace `<id>` with a complete guild, category, supported message channel, thread parent, or thread ID. Exact thread IDs can be validated regardless of their active or archived state; profile exclusions are applied later during discovery and cleanup.
If you want multiple IDs, use a comma-separated list such as `include_ids=123456789012345678,234567890123456789`.
